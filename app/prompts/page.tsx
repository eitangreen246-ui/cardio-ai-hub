...
function PromptCard({ prompt, index }: { prompt: Prompt; index: number }) {
  ...
  return (
    <div
      className="card rise group relative flex flex-col p-6"
    ...
    >
      ...
      <div className="mt-auto flex flex-wrap items-center gap-x-3 gap-y-2 pt-4">
        ...
        <span className="inline-flex items-center gap-1.5">
          <Stars value={avg} />
          <span className="text-[11px] font-bold text-faint">{avg ? avg.toFixed(1) : "—"}</span>
        </span>
        <span className="ml-auto text-[11px] font-semibold text-faint">copied {prompt.copy_count}×</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <span className="eyebrow badge-blue">Shared library</span>
          <h1 className="mt-3 font-display text-3xl font-extrabold tracking-tight">Prompts</h1>
        </div>
        <NewPromptButton />
      </div>
      <div className="mt-5">
