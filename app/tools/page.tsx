...
function ToolCard({ tool, index }: { tool: Tool; index: number }) {
  ...
  return (
    <div className="card rise flex flex-col p-6" style={{ animationDelay: `${Math.min(index, 8) * 50}ms` }}>
      <div className="flex items-start justify-between gap-3">
        <h2 className="font-display text-lg font-bold leading-snug tracking-tight">{tool.name}</h2>
        <span className={`badge shrink-0 ${ready ? "badge-success" : "badge-amber"}`}>
          <span
            className="size-1.5 rounded-full"
            style={{ background: ready ? "var(--success)" : "var(--amber)" }}
          />
      ...
      <div className="mt-auto flex flex-col gap-3 pt-4">
        ...
        <div className="flex items-center justify-between border-t pt-3" style={{ borderColor: "var(--line)" }}>
          <span className="badge badge-violet">{tool.category}</span>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <span className="eyebrow badge-violet">What we already have</span>
          <h1 className="mt-3 font-display text-3xl font-extrabold tracking-tight">AI tools</h1>
        </div>
        <NewToolButton />
      </div>
      {dbError && <DbNotice />}
      {!dbError && tools.length === 0 && (
        <div className="card mt-6 p-10 text-center">
          <p className="font-display text-lg font-bold">No tools yet</p>
